...
def page_indexing(inputpdf, vision_url, iam_token, folder_id, es, pdfname, pageNum):
...
def search_pages(page, tags, pdfname):
    inputpdf = PdfFileReader(open(str(page) + ".pdf", "rb"))
    ...
    answer = page_indexing(inputpdf, vision_url,
                           iam_token, folder_id, es, pdfname, page)
...
async def search_in_download_doc(inputpdf, tags, pdfname):
    for page in range(inputpdf.getNumPages()):
        print(page)
        writer = PdfFileWriter()
        writer.addPage(inputpdf.getPage(page))
    ...
    futures = [
        loop.run_in_executor(executor, search_pages, page, tags, pdfname)
        for page in range(inputpdf.getNumPages())]
